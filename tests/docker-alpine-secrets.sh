...
result=$(docker run --rm -it --name "$name" -v "$scriptPath":/root/deepce.sh -e MYSQL_PASSWORD=S00perS3rect alpine /root/deepce.sh -nn)
...
# Check output contains the environment secret
if ! echo "$result" | grep -q "S00perS3rect"; then
